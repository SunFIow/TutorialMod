...
public class MyCustomLauncher1 {

	private static String customName = "Dev15";
	private static boolean isCustomName = false;
	...
	public static void main(String... args) throws InterruptedException {
		...
		if (target.contains("client")) {
			...
			if (assets == null || !new File(assets).exists()) {
				throw new IllegalArgumentException(
						"Environment variable 'assetDirectory' must be set to a valid path.");
			}
			...
			if (!lst.hasValue("accessToken")) {
				if (!login(lst)) {
					...
					if (username != null) { // Replace '#' placeholders with random numbers
						...
						if (isCustomName) {
							lst.putLazy("username", customName);
						} else {
							lst.putLazy("username", "Dev" + new Random().nextInt(100));
						}
					}
		...
		if (Arrays.asList("fmldevclient", "fmldevserver", "fmldevdata", "fmluserdevclient", "fmluserdevserver")
				.contains(target)) {
			// nop
		} else {
	...
	private static String getRandomNumbers(int length) { // Generate a time-based random number, to mimic how
															// n.m.client.Main works
	...
	/**
	 * Basic implementation of Mojang's 'Yggdrasil' login system, purely intended as
	 * a dev time bare bones login. Login errors are not handled. Do not use this
	 * unless you know what you are doing and must use it to debug things REQUIRING
	 * authentication. Forge is not responsible for any auth information passed in,
	 * saved to logs, run configs, etc... BE CAREFUL WITH YOUR LOGIN INFO
	 */
