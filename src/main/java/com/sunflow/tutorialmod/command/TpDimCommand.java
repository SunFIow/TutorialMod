...
//				.executes(ctx -> run(ctx));
	...
	public int run(CommandContext<CommandSource> context) throws CommandSyntaxException {
		...
		if (player.dimension.equals(ModDimensions.BADLANDS_TYPE))
			TeleportationTools.teleportToDimension(player, DimensionType.OVERWORLD, new BlockPos(player.posX, 200, player.posZ));
		else if (player.dimension.equals(DimensionType.OVERWORLD))
			TeleportationTools.teleportToDimension(player, DimensionType.THE_NETHER, new BlockPos(player.posX, 200, player.posZ));
		else
			TeleportationTools.teleportToDimension(player, ModDimensions.BADLANDS_TYPE, new BlockPos(player.posX, 200, player.posZ));
