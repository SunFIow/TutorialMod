...
public class CustomBigTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
	private final BlockState log;
	private final BlockState leaves;

	public CustomBigTreeFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> configFactoryIn, boolean doBlockNotifyIn, BlockState log, BlockState leaves) {
	...
	private void makeFoliage(IWorldGenerationReader worldIn, int p_208525_2_, BlockPos pos, List<CustomBigTreeFeature.FoliageCoordinates> p_208525_4_, MutableBoundingBox p_208525_5_, Set<BlockPos> changedBlocks) {
		for (CustomBigTreeFeature.FoliageCoordinates bigtreefeature$foliagecoordinates : p_208525_4_) {
	...
	private void makeBranches(Set<BlockPos> p_208524_1_, IWorldGenerationReader p_208524_2_, int p_208524_3_, BlockPos p_208524_4_, List<CustomBigTreeFeature.FoliageCoordinates> p_208524_5_, MutableBoundingBox p_208524_6_) {
		for (CustomBigTreeFeature.FoliageCoordinates bigtreefeature$foliagecoordinates : p_208524_5_) {
	...
	public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldIn, Random rand, BlockPos position, MutableBoundingBox boundsIn) {
		...
		if (i == -1) {
			...
			double d0 = 1.0D;
			int k = (int) (1.382D + Math.pow(d0 * i / 13.0D, 2.0D));
			if (k < 1) {
				k = 1;
			}
			...
			int i1 = i - 5;
			List<CustomBigTreeFeature.FoliageCoordinates> list = Lists.newArrayList();
			list.add(new CustomBigTreeFeature.FoliageCoordinates(position.up(i1), l));
			...
			for (; i1 >= 0; --i1) {
				...
				if (!(f < 0.0F)) {
					for (int j1 = 0; j1 < k; ++j1) {
						double d1 = 1.0D;
						double d2 = d1 * f * (random.nextFloat() + 0.328D);
						...
						if (this.makeLimb(changedBlocks, worldIn, blockpos, blockpos1, false, boundsIn) == -1) {
							...
							if (this.makeLimb(changedBlocks, worldIn, blockpos2, blockpos, false, boundsIn) == -1) {
								list.add(new CustomBigTreeFeature.FoliageCoordinates(blockpos, blockpos2.getY()));
							}
						}
					}
				}
